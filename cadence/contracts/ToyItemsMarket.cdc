import ToyCoin from "./ToyCoin.cdc"
import ToyItems from "./ToyItems.cdc"
import FungibleToken from "./FungibleToken.cdc"
import ToyNonFungibleToken from "./ToyNonFungibleToken.cdc"

/*
    This is a simple ToyItems initial sale contract for the DApp to use
    in order to list and sell ToyItems.
...
pub contract ToyItemsMarket {
    ...
    // A sale offer has been changed, change price.
    pub event SaleOfferChanged(itemID: UInt64, newPrice: UFix64)
    ...
    // 交易市场总供应
    pub var totalSupply: {Address : Address}
    ...
    // SaleOffer
    // A ToyItems NFT being offered to sale for a set fee paid in ToyCoin.
    //
    pub resource SaleOffer: SaleOfferPublicView {
        ...
        access(self) let sellerItemProvider: Capability<&ToyItems.Collection{ToyNonFungibleToken.Provider}>

        // The ToyCoin vault that will receive that payment if teh sale completes successfully.
        access(self) let sellerPaymentReceiver: Capability<&ToyCoin.Vault{FungibleToken.Receiver}>

        // Called by a purchaser to accept the sale offer.
        // If they send the correct payment in ToyCoin, and if the item is still available,
        // the ToyItems NFT will be placed in their ToyItems.Collection .
        //
        pub fun accept(
            buyerCollection: &ToyItems.Collection{ToyNonFungibleToken.Receiver},
            ...
            //emit SaleOfferAccepted(itemID: self.item.id)
        }
        ...
        destroy() {
            ...
            // emit SaleOfferFinished(itemID: self.item.id)
        }

        // initializer
        // Take the information required to create a sale offer, notably the capability
        // to transfer the ToyItems NFT and the capability to receive ToyCoin in payment.
        //
        // init(
        //     sellerItemProvider: Capability<&ToyItems.Collection{ToyNonFungibleToken.Provider}>,
        //     itemID: UInt64,
        //     typeID: UInt64,
        //     sellerPaymentReceiver: Capability<&ToyCoin.Vault{FungibleToken.Receiver}>,
        //     price: UFix64,
        //     metadata: String
        // ) { // item: &ToyItems.NFT,
        init(
    ...
    pub resource interface CollectionManager {
        pub fun insert(offer: @ToyItemsMarket.SaleOffer)
    ...
    pub resource interface CollectionPublic {
        ...
        pub fun borrowSaleOffers(): {UInt64: &SaleOffer{SaleOfferPublicView}?}
        pub fun purchase(
            itemID: UInt64,
            buyerCollection: &ToyItems.Collection{ToyNonFungibleToken.Receiver},
            buyerPayment: @FungibleToken.Vault
    ...
    pub resource Collection : CollectionManager, CollectionPurchaser, CollectionPublic {
        ...
        //
         pub fun insert(offer: @ToyItemsMarket.SaleOffer) {
            ...
            // let itemID: UInt64 = offer.item.id
            ...
            if self.saleOffers.keys.length > 0 && !ToyItemsMarket.totalSupply.containsKey(self.owner?.address!) {
                ToyItemsMarket.totalSupply[self.owner?.address!] = self.owner?.address!
            }

            //totalSupply[itemID] = &self.saleOffers[itemID] as &SaleOffer{SaleOfferPublicView}
        ...
        pub fun remove(itemID: UInt64): @SaleOffer {
            ...
            if self.saleOffers.keys.length == 0 && ToyItemsMarket.totalSupply.containsKey(self.owner?.address!) {
                ToyItemsMarket.totalSupply.remove(key: self.owner?.address!)
            }
        ...
        // purchase
        // If the caller passes a valid itemID and the item is still for sale, and passes a ToyCoin vault
        // typed as a FungibleToken.Vault (ToyCoin.deposit() handles the type safety of this)
        ...
        pub fun borrowSaleOffers(): {UInt64: &SaleOffer{SaleOfferPublicView}?} {
            var saleOffers: {UInt64: &SaleOffer{SaleOfferPublicView}?} = {}

            for key in self.saleOffers.keys {
                saleOffers[key] = self.borrowSaleItem(itemID: key);
            }
    ...
    init () {
        ...
        self.CollectionStoragePath = /storage/d1eaadd012fcd9f0ToyItemsMarketCollection004
        self.CollectionPublicPath = /public/d1eaadd012fcd9f0ToyItemsMarketCollection004
